...
`;
  ...
  fs.writeFileSync(
    `${claudePath}.cmd`,
    `@echo off\r\n"${process.execPath}" "%~dp0${path.basename(claudePath)}" %*\r\n`,
    "utf8"
  );
  return claudePath;
}
...
function createHookEnvironment(options = {}) {
  ...
  const fakeClaude = createClaude ? createFakeClaudeBinary(binDir) : null;
  if (initGit) {
...
describe("hooks", () => {
  ...
  it("stop-review hook allows stop to continue while noting a running same-session job", () => {
    ...
    try {
      ...
      assert.equal(result.stdout.trim(), "");
      assert.match(result.stderr, /stop-time review passed/i);
      assert.match(result.stderr, /Claude Code is still running/);
      assert.doesNotMatch(result.stderr, /running-review-job/);
    } finally {
      cleanupHookEnvironment(testEnv);
    }
  });

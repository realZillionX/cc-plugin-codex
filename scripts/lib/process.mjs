...
export function getProcessIdentity(pid) {
  ...
  if (process.platform === 'darwin') {
    const row = runCommandChecked('ps', ['-o', 'lstart=,comm=', '-p', String(numericPid)]);
    return row.stdout.trim();
  }
  if (process.platform === 'win32') {
    const row = runCommandChecked('powershell', [
      '-NoProfile',
      '-NonInteractive',
      '-Command',
      `(Get-CimInstance Win32_Process -Filter "ProcessId = ${numericPid}").CreationDate`
    ]);
    return row.stdout.trim();
  }

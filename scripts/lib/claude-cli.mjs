...
const CLAUDE_COMMAND_ENV = "CLAUDE_COMPANION_CLAUDE_COMMAND_JSON";
...
function resolveClaudeInvocation(args) {
  const rawOverride = process.env[CLAUDE_COMMAND_ENV];
  if (rawOverride) {
    try {
      const commandParts = JSON.parse(rawOverride);
      if (
        Array.isArray(commandParts) &&
        commandParts.length > 0 &&
        commandParts.every((part) => typeof part === "string")
      ) {
        const [command, ...prefixArgs] = commandParts;
        return { command, args: [...prefixArgs, ...args] };
      }
    } catch {
      // Fall back to the normal CLI name; setup/status will report failures.
    }
  }
...
export function getClaudeAvailability(cwd) {
  const invocation = resolveClaudeInvocation(["--version"]);
  try {
...
export function getClaudeAuthStatus(cwd) {
  ...
  const invocation = resolveClaudeInvocation(["auth", "status"]);
  try {
    const result = spawnSync(invocation.command, invocation.args, {
      cwd,
      encoding: "utf8",

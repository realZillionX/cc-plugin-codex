...
function isPublicProgressLine(line) {
  return (
    /^Starting Claude/i.test(line) ||
    /^Queued/i.test(line) ||
    /^Claude Code is responding\.?$/i.test(line) ||
    /^Using tool:/i.test(line) ||
    /^Running tool\.?$/i.test(line) ||
    /^API retry/i.test(line) ||
    /^Process cancelled/i.test(line) ||
    /^Cancel attempt failed/i.test(line) ||
    /^Cancel result:/i.test(line)
  );
}
...
function resolveLastActivityAt(job, logFile) {
  if (logFile && fs.existsSync(logFile)) {
    try {
      return fs.statSync(logFile).mtime.toISOString();
    } catch {}
  }
  return job.updatedAt ?? job.completedAt ?? job.startedAt ?? job.createdAt ?? null;
}

function formatLastActivity(value) {
  const elapsed = formatElapsedDuration(value);
...
function inferJobPhase(job, progressPreview = []) {
  ...
  for (let i = progressPreview.length - 1; i >= 0; i--) {
    ...
    if (line === "claude code is responding.") return "responding";
    if (line.startsWith("using tool:") || line === "running tool.") return "tool";
    if (line.startsWith("api retry")) return "retrying";
...
export function enrichJob(job, options = {}) {
  ...
  const lastActivityAt = resolveLastActivityAt(job, managedLogFile);
  return {
    ...enriched,
    lastActivityAt,
...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  if (prefixMatches.length > 1) throw new Error(`Claude Code process reference "${reference}" is ambiguous. Check $cc:status before choosing a process.`);
  throw new Error(`No Claude Code process found for "${reference}". Run $cc:status to list known processes.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) throw new Error(`No Claude Code process found for "${reference}".`);
  return {
    workspaceRoot,
...
export function resolveResultJob(cwd, reference) {
  ...
  if (reference) {
    ...
    throw new Error(`Claude Code process is ${enriched.status}. Check $cc:status for more details.`);
  }
...
export function resolveCancelableJob(cwd, reference) {
  ...
  if (activeJobs.length > 1) throw new Error("Multiple Claude Code processes are active. Check $cc:status before cancelling.");
